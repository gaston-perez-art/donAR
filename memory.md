...
- **Prototipo implementado (pendiente de veredicto visual de Gastón en su iPhone):** `expo-blur@15.0.8` instalado (`npx expo install`, compatible SDK 54, corre en Expo Go). `donar-tab-bar.tsx` reescrito: barra flotante `position:absolute` (para que el feed scrollee por detrás y el blur tenga qué difuminar; sin eso no se ve el efecto), `BlurView` intensity 40 tint light + capa de tinte translúcido `rgba(255,255,255,0.55)` para contraste de labels, hairline top. El `+` central sigue flotando por encima del vidrio (queda bien). Paridad: `experimentalBlurMethod="dimezisBlurView"` para que Android sí bluree (sin eso Android solo muestra el tinte). Las pantallas ya reservaban `TabBarHeight` abajo, así que nada queda tapado; de paso, hacer la barra absolute corrige un posible doble-espaciado que había antes (react-navigation reservaba espacio + el padding de las screens). `tsc` + `expo export` limpios (1066 módulos, +2 de blur). **NO pusheado: espera OK visual de Gastón.** Si le gusta, próximo paso llevar el mismo lenguaje al header y evaluar la variante "pill flotante".

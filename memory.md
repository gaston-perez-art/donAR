...
- **Bug del carrusel no swipeable desde el feed (backlog, 27 jul): resuelto.** La hipótesis original del backlog (el `Pressable` de la tarjeta interceptando el gesto) no era la causa real: es el bug conocido de RN en Android de ScrollView horizontal anidado dentro de un ScrollView vertical (la tarjeta vive dentro del scroll del feed), que necesita el prop `nestedScrollEnabled`. Sumado a `cause-card.tsx` (el caso reportado) y, por el mismo motivo estructural, a `cause/[id].tsx` (mismo patrón, no reportado roto pero mismo riesgo). `npx tsc --noEmit` limpio. **Pendiente de Gastón: probar el swipe del carrusel en el feed y en el detalle, en Android.**

...
**Cuarta idea volada, sin validar (30 jul, propuesta de Gastón).** Intercalar "cards de publicidad" entre las causas del feed, estilo Tinder (que intercala anuncios entre perfiles para swipear). A diferencia de la idea anterior, acá quien paga no es una causa por más visibilidad que otra: es una marca externa, coherente con el "ingreso secundario (fase 2)" ya documentado arriba (sponsors que pagan por acceder a la comunidad de donantes activos). La tensión a cuidar es otra: mezclar contenido comercial con un feed de necesidades reales puede sentirse tono-sordo si no se cuida el detalle (frecuencia baja, marcas coherentes con lo solidario, nunca compitiendo visualmente ni confundiéndose con una causa real). Sin desarrollar ni decidir.

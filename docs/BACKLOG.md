...
- ✅ **El carrusel de fotos no se puede swipear desde el feed.** Reportado 27 jul, resuelto 28 jul. La sospecha original (el `Pressable` de la tarjeta interceptando el gesto) no era la causa: es un problema conocido de React Native en Android, ScrollView horizontal anidado dentro de un ScrollView vertical (la tarjeta vive dentro del feed, que también scrollea) no funciona sin `nestedScrollEnabled`. Fix: sumado ese prop al carrusel de `cause-card.tsx` y, por el mismo motivo estructural, al de `cause/[id].tsx` (no reportado roto, pero mismo patrón).

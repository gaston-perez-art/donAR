...
- ⬜ **3.5** **Escalabilidad de la lista de transferencias/aportes (idea de Gastón, 29 jul).** Hoy `cause/[id].tsx` renderiza cada transferencia pendiente como una tarjeta completa con la foto del comprobante en tamaño real, inline en el scroll — si una causa recibe miles de aportes, se vuelve inmanejable (carga todas las imágenes a la vez, sin paginar). Mismo problema, más leve, en "Aportes recibidos" (una fila por aporte confirmado, sin límite). Propuesta de Gastón: fila compacta (nombre + monto), el comprobante se ve **tocando** (modal o pantalla aparte), no expandido en línea; confirmar/rechazar rápido desde la fila. No bloquea el MVP (volumen real hoy es bajísimo), pero conviene resolverlo antes de escalar.

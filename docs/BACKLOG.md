...
- ⬜ **4.1** Estado de cierre: "cumplida" (happy) o "cerrada sin llegar" (unhappy). **DECIDIDO (29 jul):** el cierre se dispara por **monto** (lo confirmado ≥ meta → cumplida) o por **tiempo** (venció el plazo → cerrada sin llegar). Al cumplirse por monto, la causa **corta donaciones nuevas** (se cierra de verdad); los aportes en vuelo (`pending`, ya transferidos) se completan igual aunque pasen un poco la meta ("una vez que pase, genial, pero ahí ya está"). Esto **reemplaza** la regla vieja "meta = objetivo, no techo" del 24 jul (ver Decisiones). Motivo de Gastón: sin cierre por monto las causas no tienen fin.
- ⬜ **4.2** Mini-reporte de cierre: cuánto se juntó, cuántas personas, en cuánto tiempo.
- ⬜ **4.3** Flujo de agradecimiento **DISEÑADO (29 jul, pedido enfático de Gastón: "el cierre emocional del producto").** Disparador: la causa se cierra (cumplida o vencida; cambia el tono, no el flujo). Del lado del beneficiado: (a) **mensaje de cierre público** — qué logró + foto opcional del resultado (reusa el patrón de subir foto), queda en el detalle de la causa cerrada; (b) **gracias general** que les llega a todos los donantes; (c) **gracias puntual opcional** a un donante específico (ya ve los aportes confirmados con nombre). Del lado del donante: en su tab Actividad / "lo que doné", la causa pasa a **cumplida** y aparece el agradecimiento (cierra su arco: mi aporte sirvió, me lo reconocieron). **Dependencia:** el aviso ideal sería push, bloqueado en Expo Go → por ahora vive in-app en Actividad; el push real entra con el dev build.

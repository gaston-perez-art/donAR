...
- ❓ **Riesgo de identidad huérfana (29 jul, descubierto en testing real, DATO CONFIRMADO Y REASIGNADO).** El hermano de Gastón donó por transferencia a "Me muero de hambre" (causa de Gastón); a Gastón no le aparecía nada para confirmar. Causa raíz confirmada: se creó bajo una sesión anónima que después se cerró (`signOut`) sin vincular mail a ESA sesión puntual. Gastón ya corrió el fix de datos: `owner_id` de "Me muero de hambre" ahora apunta a su cuenta real (`gaston.product.expert@gmail.com`, `is_anonymous: false`). **Pero seguía sin ver la donación**, porque su sesión ACTIVA en el celular era otra (anónima, sin vincular) — el fix de datos no alcanza si la sesión del dispositivo no cambia con él. Aclarado: el login (`loginWithEmail`) YA EXISTE y de hecho `AccountLink` en `profile.tsx` YA lo dispara automáticamente si el mail ya está registrado (no hace falta un botón "loguear" separado del de "vincular"). Sin resolver como decisión de producto: ¿bloqueamos crear una causa sin mail vinculado? ¿avisamos antes de cerrar sesión si hay causas propias sin vincular? Riesgo real para usuarios reales, no solo testing.
